...
import { searchPAndD } from '@/api/purchase'
  ...
  data () {
    return {
      ...
      columns: [
        ...
        { title: '当时库存', key: 'atTimeStock', sortable: true, width: 110, align: 'center' },
        { title: '入库价格', key: 'purchasePrice', sortable: true, width: 110, align: 'center' },
        { title: '入库总价', key: 'purchaseTotal', sortable: true, width: 110, align: 'center' },
      ...
      searchPAndD(data, pages).then(res => {
        if (res.code === 200) {
          ...
          this.data = data.map(item => {
            ...
            item['purchaseStock'] = Number(item['purchaseStock'])
            item['purchasePrice'] = Number(item['purchasePrice'])
            item['stock'] = Number(item['stock'])
            item['price'] = Number(item['price'])
            item['purchaseTotal'] = Number(item['purchaseStock']) * Number(item['purchasePrice'])
